...
def backup_device(host: str, username: str, password: str,
    ...
    # =========================================================
    # 🔍 DETECCIÓN PREVIA (opcional por comportamiento)
    # =========================================================
    log("Iniciando conexión...", "info")
    ...
    try:
        ...
        # =====================================================
        # 🔴 DETECCIÓN DE EQUIPO LEGACY (KEX SHA1)
        # =====================================================
    ...
    # =========================================================
    # 🟡 SEGUNDO INTENTO SCRAPLI (modo legacy opcional)
    # =========================================================
    if not config and not is_legacy:
        try:
            ...
            conn = Scrapli(
                ...
                auth_strict_key=False,
                platform="cisco_iosxe",
                transport="paramiko",  # 👈 clave
                ssh_config_file=True,  # 👈 usa ~/.ssh/config
                timeout_socket=timeout,
                timeout_transport=timeout,
            )
    ...
    # =========================================================
    # 🟢 NETMIKO (FALLBACK FINAL INTELIGENTE)
    # =========================================================
    if not config:
        try:
    ...
    try:
        ...
        filename = f"{hostname}_{host.replace('.', '_')}_{timestamp}.txt"
...
class CiscoBackupApp(tk.Tk):
    ...
    def _configure_syslog(self):
     from syslog_manager import configure_syslog

     syslog_ip = "192.168.99.1"  # puedes hacerlo dinámico luego

     def worker():
        for entry, badge, ip in self._devices:
            result = configure_syslog(
                ip,
                self.username.get(),
                self.password.get(),
                syslog_ip,
                secret=self.enable.get(),
    ...
    def _build_actions(self, parent):
        ...
        IconButton(row5, "📡 Configurar Syslog",
                   color="#1f3a5f",
                   hover_color="#2a4d7a",
                   command=self._configure_syslog).pack(fill="x")
    ...
    # 6. Log final
     self.btn_snmpself._log("Sistema reiniciado correctamente.", "dim")    
    ...
    def _open_snmp_monitor(self):
     ...
     if not ENABLE_SNMP:
        self._log("Módulo SNMP deshabilitado en esta versión.", "warning")
        return
    ...
    def _show_about(self):
        ...
        win.title("Acerca de — Cisco Backup Tool v1")
